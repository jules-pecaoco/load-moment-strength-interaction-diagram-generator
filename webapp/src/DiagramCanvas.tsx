...
import { calibrate, toPixel, evaluateConditionalLimits, resolveBoundary } from './lib/math';
...
export const DiagramCanvas: React.FC<DiagramCanvasProps> = ({ kn, rn, config }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const transform = calibrate(p1.pixel as [number, number], p1.value as [number, number], p2.pixel as [number, number], p2.value as [number, number]);
      ...
      if (lStyle.visible) {
        ctx.font = `bold ${lStyle.fontsize}px sans-serif`;
  ...
  return (
    <div className="canvas-container">
      {!image && <div className="loading">Loading image...</div>}
      <canvas 
        ref={canvasRef} 
